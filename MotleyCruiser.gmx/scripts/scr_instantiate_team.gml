...
var xx = argument0;
var yy = argument1;
var thiscrew = noone;// object
var crewData = noone;// array
for(var i=0; i<array_length_1d(global.team); i++){
    if(global.team[i]== noone) continue;
    
    crewData = scr_get_array_1d(global.crew,global.team[i]);
    if(!is_array(crewData)){
        continue;
    }
    var npc_data = scr_npc();// Do this in case we've added any extra attributes since the last game save.
    npc_data = scr_fill_array(npc_data,crewData);
