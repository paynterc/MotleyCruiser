...
if(ttl_price<=global.credits){
    var result = scr_inv_item_add(item_id,amount);// result will be the amount that would not fit in cargo. probably zero.
    global.credits -= (amount-result) * ttl_price;
    if(result!=0){
        scr_display_message("You don't have enough cargo space.","");
    }
