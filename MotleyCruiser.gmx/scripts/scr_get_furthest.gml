/// Find the furthest instance of a given object to the given source
// scr_get_furthest(ObjectTarget, ObjSource, RangeLimit);
...
for(var i = 0; i < instance_number(obj_target); i++) {
    ...
    if(this_range<range || range<0){
        if(furthest==noone){
            furthest = this_obj;
            furthest_range = this_range;
        }else if(this_range>furthest_range){
            furthest = this_obj;
            furthest_range = this_range;
        }
    }
