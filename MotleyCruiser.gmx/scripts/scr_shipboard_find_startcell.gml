/// scr_shipboard_find_startcell(ship_data)
...
for(var m=0; m<array_length_1d(moduleData); m++){
    ...
    var r = coords[0];
    var c = coords[1];
    allCells = scr_push_array( allCells,scr_array(c,r) );
        var check=1;
    if(c != center) continue;
    if(r==center && c==center){
        centerCells = noone;
        centerCells[0] = scr_array(c,r);
        break;
    }else{
        // if mag3. center = 1. If col = 0, row=0, return -1,-1
        centerCells = scr_push_array( centerCells,scr_array(c,r) );
    }
}
